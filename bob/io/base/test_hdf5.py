...
def read_write_check(outfile, dname, data, dtype=None):
    ...
    assert numpy.array_equal(
        outfile.get_attribute(dname + "_single_attr", dname + "_single"), data[0]
    )
    ...
    for i, b in enumerate(back):
        assert numpy.array_equal(b, data[i])

    # Check the attributes
    assert numpy.array_equal(
        outfile.get_attribute(dname + "_attr", dname + "_single"), data
    )
...
def test_append_compression():
    ...
    try:
        ...
        for k in range(len(data)):
            outfile.append("data", data[k], compression=9)
        recovered = outfile.read("data")
        assert numpy.array_equal(data, recovered)
...
def test_from_dict_to_hdf5():
    ...
    def compare(ref_dict, comp_dict):
        ...
        assert ref_dict["string"] == comp_dict["string"]
        assert numpy.allclose(ref_dict["array"], comp_dict["array"])
        assert numpy.allclose(ref_dict["list"], comp_dict["list"])
